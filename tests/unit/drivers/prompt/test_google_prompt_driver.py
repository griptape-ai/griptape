...
class TestGooglePromptDriver:
    ...
    def test_try_run(
        ...
        driver = GooglePromptDriver(
            ...
            native_structured_output_mode="tool",
    ...
    def test_verify_native_structured_output_mode(self):
        assert GooglePromptDriver(model="foo", native_structured_output_mode="tool")

        with pytest.raises(ValueError, match="GooglePromptDriver does not support `native` structured output mode."):
            GooglePromptDriver(model="foo", native_structured_output_mode="native")
