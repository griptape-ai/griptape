...
class TestHuggingFaceHubPromptDriver:
    ...
    def test_verify_native_structured_output_mode(self):
        assert HuggingFaceHubPromptDriver(model="foo", api_token="bar", native_structured_output_mode="native")

        with pytest.raises(
            ValueError, match="HuggingFaceHubPromptDriver does not support `tool` structured output mode."
        ):
            HuggingFaceHubPromptDriver(model="foo", api_token="bar", native_structured_output_mode="tool")
