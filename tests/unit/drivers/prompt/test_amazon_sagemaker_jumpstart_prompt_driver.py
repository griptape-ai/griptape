...
class TestAmazonSageMakerJumpstartPromptDriver:
    ...
    def test_init(self):
        assert AmazonSageMakerJumpstartPromptDriver(endpoint="foo")

    def test_try_run(self, mock_client):
        # Given
        driver = AmazonSageMakerJumpstartPromptDriver(endpoint="model")
        ...
        mock_client.invoke_endpoint.assert_called_with(
            ...
            Body=json.dumps(
                {
                    "inputs": "foo\n\nUser: bar",
                    "parameters": {"temperature": driver.temperature, "max_new_tokens": 250, "do_sample": True},
                }
            ),
