...
class TestBasePromptDriver:
    ...
    def test_run_via_pipeline_retries_failure(self, mock_config):
        mock_config.driver_config.prompt_driver = MockFailingPromptDriver(max_failures=2, max_attempts=1)
        pipeline = Pipeline()
    ...
    def test_run_with_tools(self, mock_config):
        mock_config.driver_config.prompt_driver = MockPromptDriver(max_attempts=1, use_native_tools=True)
        pipeline = Pipeline()
