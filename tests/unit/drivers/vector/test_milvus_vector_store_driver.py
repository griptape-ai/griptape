...
class TestMilvusVectorStoreDriver:
    ...
    @pytest.mark.parametrize(
        ("kwargs", "message"),
        [
            ({"vector_dim": 0}, "vector_dim must be greater than zero"),
            ({"id_field": "invalid field"}, "Invalid Milvus field name"),
            ({"id_field": "same", "vector_field": "same"}, "field names must be unique"),
            ({"id_max_length": 0}, "max_length must be greater than zero"),
        ],
    )
    def test_rejects_invalid_configuration(self, embedding_driver, kwargs, message):
    ...
    def test_serialization_excludes_token_and_client(self, embedding_driver, client):
        driver = MilvusVectorStoreDriver(
            collection_name="test_collection",
            embedding_driver=embedding_driver,
            token="secret-token",
    ...
    @pytest.mark.parametrize("reserved_field", ["id", "text", "metadata", "artifact"])
    def test_upsert_vector_rejects_reserved_kwarg(self, driver, client, reserved_field):
        with pytest.raises(ValueError, match="cannot override reserved fields"):
            driver.upsert_vector([1.0, 0.0], **{reserved_field: "override"})
    ...
    def test_load_entries_without_namespace(self, driver, client):
        ...
        assert [entry.id for entry in entries] == ["vector-id-1", "vector-id-2"]

    def test_load_entries_closes_iterator_on_failure(self, driver, client):
        client.has_collection.return_value = True
        iterator = client.query_iterator.return_value
        iterator.next.side_effect = RuntimeError("query failed")
    ...
    def test_query_vector_returns_empty_when_collection_is_missing(self, driver, client):
        client.has_collection.return_value = False

        assert driver.query_vector([1.0, 0.0]) == []
    ...
    def test_existing_collection_rejects_auto_id(self, driver, client):
        client.has_collection.return_value = True
        collection_description = self._collection_description(vector_dim=2)
        collection_description["auto_id"] = True
        client.describe_collection.return_value = collection_description

        with pytest.raises(ValueError, match="auto_id disabled"):
            driver.upsert_vector([1.0, 0.0])

        client.upsert.assert_not_called()

    def test_existing_collection_requires_dynamic_fields(self, driver, client):
        client.has_collection.return_value = True
        collection_description = self._collection_description(vector_dim=2)
        collection_description["enable_dynamic_field"] = False
        client.describe_collection.return_value = collection_description

        with pytest.raises(ValueError, match="dynamic fields enabled"):
            driver.upsert_vector([1.0, 0.0])

        client.upsert.assert_not_called()

    def test_existing_collection_rejects_field_limit_mismatch(self, driver, client):
        client.has_collection.return_value = True
        collection_description = self._collection_description(vector_dim=2)
        collection_description["fields"][0]["params"]["max_length"] = 1024
        client.describe_collection.return_value = collection_description

        with pytest.raises(ValueError, match="max_length is 1024, expected 512"):
            driver.upsert_vector([1.0, 0.0])

        client.upsert.assert_not_called()

    def test_existing_collection_requires_nullable_namespace(self, driver, client):
        client.has_collection.return_value = True
        collection_description = self._collection_description(vector_dim=2)
        collection_description["fields"][3]["nullable"] = False
        client.describe_collection.return_value = collection_description

        with pytest.raises(ValueError, match="namespace.*must be nullable"):
            driver.upsert_vector([1.0, 0.0])

        client.upsert.assert_not_called()

    def test_existing_collection_requires_vector_index(self, driver, client):
        client.has_collection.return_value = True
        client.list_indexes.return_value = []

        with pytest.raises(ValueError, match="must have an index"):
            driver.upsert_vector([1.0, 0.0])

        client.upsert.assert_not_called()

    def test_existing_collection_rejects_index_metric_mismatch(self, driver, client):
        client.has_collection.return_value = True
        client.describe_index.return_value["metric_type"] = "L2"

        with pytest.raises(ValueError, match="index metric is incompatible"):
            driver.upsert_vector([1.0, 0.0])
    ...
    def _collection_description(vector_dim: int) -> dict:
        ...
        return {
            ...
            "fields": [
                ...
                {"name": "namespace", "type": DataType.VARCHAR, "nullable": True, "params": {"max_length": 512}},
                {"name": "metadata", "type": DataType.JSON, "nullable": True, "params": {}},
            ],
        }
