...
from griptape.converters import ChatgptPluginConverter
from griptape.executors import LocalExecutor
from tests.mocks.mock_tool.tool import MockTool


class TestChatgptPluginConverter:
    @pytest.fixture
    def tool(self):
        return MockTool(
            test_field="hello"
    ...
    def test_full_host(self, tool, executor):
        assert ChatgptPluginConverter(executor=executor, host="foo", path_prefix="/bar").full_host == "foo/bar"

    def test_generate_manifest(self, tool, executor):
        converter = ChatgptPluginConverter(executor=executor, host="foo")
        manifest = converter.generate_manifest(tool)
        ...
        assert manifest["api"]["url"] == f"{converter.full_host}{converter.OPENAPI_SPEC_FILE}"
        assert manifest["api"]["is_user_authenticated"] is False
        assert manifest["logo_url"] == f"{converter.full_host}logo.png"
    ...
    def test_generate_api_spec(self, tool, executor):
        converter = ChatgptPluginConverter(executor=executor, host="foo")
        api = converter.generate_api(tool)

        assert isinstance(converter.generate_api_spec(api), str)

    def test_generate_api(self, tool, executor):
        converter = ChatgptPluginConverter(executor=executor, host="foo")
        assert isinstance(converter.generate_api(tool), FastAPI)