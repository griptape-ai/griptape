...
class TestOpenStreetMapTool:
    ...
    @patch.object(OpenStreetMapTool, '_search', return_value={"location": "test_location"})
    ...
    @patch.object(OpenStreetMapTool, '_search', side_effect=Exception("An error occurred while searching for the location."))
    def test_search_exception(self, mock_search, tool):
        params = {"query": "test_location"}
        with pytest.raises(Exception) as e:
            tool.search(params)
        
        assert str(e.value) == "An error occurred while searching for the location."
