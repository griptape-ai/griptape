...
class TestAwsIamClient:
    ...
    def test_get_user_policy(self):
        ...
        assert (
            ...
            in AwsIamClient(session=boto3.Session(), off_prompt=False).get_user_policy({"values": value}).value
        )

    def test_list_mfa_devices(self):
        assert (
            "error listing mfa devices"
            in AwsIamClient(session=boto3.Session(), off_prompt=False).list_mfa_devices({}).value
        )
    ...
    def test_list_user_policies(self):
        ...
        assert (
            ...
            in AwsIamClient(session=boto3.Session(), off_prompt=False).list_user_policies({"values": value}).value
        )

    def test_list_users(self):
        assert "error listing s3 users" in AwsIamClient(session=boto3.Session(), off_prompt=False).list_users({}).value
