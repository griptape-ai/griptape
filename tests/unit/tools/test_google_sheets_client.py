...
class TestGoogleSheetsClient:
    ...
    def test_list_spreadsheets(self, client):
        ...
        assert (
            "error listing spreadsheet due to malformed credentials:"
            in result.value
        )
    ...
    def test_create_spreadsheet(self, client):
        ...
        assert isinstance(result, ErrorArtifact)
        assert (
            "error creating spreadsheet due to malformed credentials"
            in result.value
        )
    ...
    def test_upload_spreadsheet(self, client):
        ...
        assert isinstance(result, ErrorArtifact)
        assert (
            "error uploading spreadsheet due to malformed credentials:"
            in result.value
        )
    ...
    def test_modify_cell(self, client):
        ...
        assert isinstance(result, ErrorArtifact)
        assert (
            "error modifying value in cell due to malformed credentials:"
            in result.value
        )
