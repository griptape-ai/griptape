...
class TestGriptapeCloudToolTool:
    ...
    def test_init(self):
        ...
        expected_activities = {
            ...
            "processEnum": {
                ...
                "description": "Processes a enum input",
                "schema": schema.Schema(
                    {schema.Optional(schema.Literal("flag", description="The enum to process")): ["true", "false"]}
                ),
            },
            "processArray": {
                "name": "processArray",
    ...
    def test_error_schema(self, mocker):
        mock_get = mocker.patch("requests.get")
        mock_get.return_value.status_code = 200
        mock_get.return_value.json.return_value = ERROR_SCHEMA
