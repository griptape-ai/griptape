...
class TestStructureRunTask:
    def test_run(self, mock_config):
        mock_config.driver_config.prompt_driver = MockPromptDriver(mock_output="agent mock output")
        agent = Agent()
        mock_config.driver_config.prompt_driver = MockPromptDriver(mock_output="pipeline mock output")
        pipeline = Pipeline()
