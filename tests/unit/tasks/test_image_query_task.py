...
class TestImageQueryTask:
    ...
    def test_bad_run(self, image_query_driver, text_artifact, image_artifact):
        with pytest.raises(ValueError, match="All inputs"):
            ImageQueryTask(("foo", [image_artifact, text_artifact]), image_query_driver=image_query_driver).run()
