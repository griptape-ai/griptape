...
class TestJsonExtractionEngine:
    ...
    def test_extract(self, engine):
        ...
        result = engine.extract_text("foo", template_schema=json_schema)
    ...
    def test_extract_error(self, engine):
        assert isinstance(engine.extract_text("foo", template_schema=lambda: "non serializable"), ErrorArtifact)
