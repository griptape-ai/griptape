...
class TestMetadataGenerationModule:
    def test_run(self):
        ...
        assert "foo" in module.run(RagContext(module_params={"metadata": "foo"}, query="test")).before_query[0]

    def test_run_with_override(self):
        module = MetadataGenerationRagModule(metadata="bar")

        assert "bar" in module.run(RagContext(module_params={"metadata": "foo"}, query="test")).before_query[0]
