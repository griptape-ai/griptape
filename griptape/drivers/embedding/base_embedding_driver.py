        ...
        for attempt in self.retrying():
            with attempt:
                if isinstance(value, str):
                    return self.embed(TextArtifact(value))
                elif isinstance(value, TextArtifact):
                    artifact_value = value.to_text()
                    if (
                        self.tokenizer is not None
                        and self.tokenizer.count_tokens(artifact_value) > self.tokenizer.max_input_tokens
                    ):
                        return self._embed_long_string(artifact_value)
                    else:
                        return self.try_embed_chunk(artifact_value)
