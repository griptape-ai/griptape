...
class MilvusVectorStoreDriver(BaseVectorStoreDriver):
    ...
    Attributes:
        ...
        vector_dim: Optional expected vector dimension. Only setup or the first upsert creates a missing collection; queries are non-mutating.
    ...
    Milvus primary keys are global to a collection. Namespaces filter entries but do not scope IDs, so upserting the
    same ID in another namespace replaces the existing entry, and ``delete_vector`` removes that ID regardless of its
    namespace.
    """
    ...
    def upsert_vector(
        ...
        **kwargs,
        ...
        self._validate_upsert_kwargs(kwargs)
    ...
    def load_entries(self, *, namespace: str | None = None) -> list[BaseVectorStoreDriver.Entry]:
        ...
        try:
            while batch := iterator.next():
                entries.extend(self._entry_from_entity(result, include_vectors=True) for result in batch)
        finally:
            iterator.close()
    ...
    def query_vector(
        ...
        **kwargs,
        ...
        self._validate_collection(vector_dim=len(vector))
    ...
    def _collection_exists(self) -> bool:
        if self._client is None and self.uri.endswith(".db") and not Path(self.uri).exists():
            return False

        return cast("bool", self.client.has_collection(collection_name=self.collection_name))
    ...
    def _validate_collection(self, *, vector_dim: int | None = None) -> None:
        ...
        self._validate_required_fields(fields, pymilvus)
        self._validate_collection_properties(collection, fields)
        self._validate_field_limits(fields)
        ...
        self._validate_vector_index()

    def _validate_required_fields(self, fields: dict[str, dict[str, Any]], pymilvus: Any) -> None:
        required_fields = {
    ...
    def _validate_collection_properties(
        self,
        ...
        primary_fields = [field["name"] for field in collection.get("fields", []) if field.get("is_primary")]
        if primary_fields != [self.id_field]:
            raise ValueError(f"Milvus collection must use {self.id_field!r} as its only primary key.")

        if collection.get("auto_id"):
            raise ValueError("Milvus collection must have auto_id disabled.")

        if not collection.get("enable_dynamic_field"):
            raise ValueError("Milvus collection must have dynamic fields enabled.")

        for field_name in (self.namespace_field, self.metadata_field):
            if not fields[field_name].get("nullable"):
                raise ValueError(f"Milvus field {field_name!r} must be nullable.")

    def _validate_field_limits(self, fields: dict[str, dict[str, Any]]) -> None:
        expected_max_lengths = {
            self.id_field: self.id_max_length,
            self.text_field: self.text_max_length,
            self.namespace_field: self.namespace_max_length,
        }
        for field_name, expected_max_length in expected_max_lengths.items():
            existing_max_length = fields[field_name].get("params", {}).get("max_length")
            if existing_max_length is None or int(existing_max_length) != expected_max_length:
                raise ValueError(
                    f"Milvus field {field_name!r} max_length is {existing_max_length}, expected {expected_max_length}."
                )

    def _validate_vector_index(self) -> None:
        index_names = self.client.list_indexes(
            collection_name=self.collection_name,
            field_name=self.vector_field,
        )
        indexes = [
            self.client.describe_index(collection_name=self.collection_name, index_name=index_name)
            for index_name in index_names
        ]
        if not indexes:
            raise ValueError(f"Milvus field {self.vector_field!r} must have an index.")

        metric_types = {
            str(index.get("metric_type", "")).upper()
            for index in indexes
            if index.get("field_name") == self.vector_field
        }
        if self.metric_type not in metric_types:
            raise ValueError(
                f"Milvus field {self.vector_field!r} index metric is incompatible with {self.metric_type!r}."
            )
    ...
    def _validate_configuration(self) -> None:
        ...
        for field_name in field_names:
            if not FIELD_NAME_RE.fullmatch(field_name):
                raise ValueError(f"Invalid Milvus field name: {field_name!r}.")
        if len(set(field_names)) != len(field_names):
            raise ValueError("Milvus field names must be unique.")
        ...
        for field_name, max_length in (
            ...
            if max_length <= 0:
                raise ValueError(f"Milvus field {field_name!r} max_length must be greater than zero.")

    def _validate_upsert_kwargs(self, kwargs: dict[str, Any]) -> None:
        collisions = sorted(self._reserved_fields().intersection(kwargs))
        if collisions:
            raise ValueError(f"Milvus upsert kwargs cannot override reserved fields: {', '.join(collisions)}.")

    def _reserved_fields(self) -> set[str]:
        return {
            self.id_field,
            self.vector_field,
            self.text_field,
    ...
    def _dynamic_fields_from_meta(self, meta: dict) -> dict[str, Any]:
        return {
            ...
            if field_name not in self._reserved_fields()
