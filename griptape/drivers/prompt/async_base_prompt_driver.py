...
@define(kw_only=True)
class AsyncBasePromptDriver(SerializableMixin, ExponentialBackoffMixin, ABC):
    """Async Base class for the Prompt Drivers.
    ...
    async def run(self, prompt_input: PromptStack | BaseArtifact) -> Message:
    ...
    async def try_stream(self, prompt_stack: PromptStack) -> AsyncIterator[DeltaMessage]:
        """Async generator that yields DeltaMessage objects."""
        yield  # type: ignore
    ...
    async def __process_run(self, prompt_stack: PromptStack) -> Message:
        return await self.try_run(prompt_stack)

    async def __process_stream(self, prompt_stack: PromptStack) -> Message:
