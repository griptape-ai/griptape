...
class AnthropicPromptDriver(BasePromptDriver):
    ...
    def try_run(self, message_stack: MessageStack) -> Message:
        response = self.client.messages.create(**self._base_params(message_stack))

        return Message(
            content=[self.__message_content_to_message_stack_content(content) for content in response.content],
    ...
    def try_stream(self, message_stack: MessageStack) -> Iterator[DeltaMessage]:
        events = self.client.messages.create(**self._base_params(message_stack), stream=True)

        for event in events:
            if event.type == "content_block_delta":
                yield DeltaMessage(content=self.__message_content_delta_to_message_stack_content_delta(event))
    ...
    def __message_content_delta_to_message_stack_content_delta(
