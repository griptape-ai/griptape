...
class AsyncGriptapeCloudPromptDriver(BasePromptDriver):
    ...
    Attributes:
        ...
        base_url: Base URL for Griptape Cloud API.
        api_key: API key for Griptape Cloud.
        headers: HTTP headers for requests.
        tokenizer: Tokenizer instance.
        client: httpx.AsyncClient for making async HTTP requests.
    """
    ...
    _client: Optional[httpx.AsyncClient] = field(default=None, kw_only=True, alias="client")
    ...
    async def async_try_run(self, prompt_stack: PromptStack) -> Message:
        ...
        logger.debug(params)
        response = await self.client.post(url, headers=self.headers, json=params)
    ...
    async def async_try_stream(self, prompt_stack: PromptStack) -> AsyncIterator[DeltaMessage]:
        ...
        params = self._base_params(prompt_stack)
        logger.debug(params)
        async with self.client.stream("POST", url, headers=self.headers, json=params) as response:
            response.raise_for_status()
