...
class HuggingFaceHubPromptDriver(BasePromptDriver):
    ...
    client: InferenceApi = field(
        default=Factory(
            lambda self: InferenceApi(repo_id=self.model, token=self.api_token, gpu=self.use_gpu), takes_self=True
        ),
        kw_only=True,
    )
    tokenizer: HuggingFaceTokenizer = field(
        default=Factory(
            lambda self: HuggingFaceTokenizer(
                tokenizer=AutoTokenizer.from_pretrained(self.model), max_tokens=self.MAX_NEW_TOKENS
            ),
            takes_self=True,
        ),
        kw_only=True,
