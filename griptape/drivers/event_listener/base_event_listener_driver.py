    ...
    def publish_event(self, event: BaseEvent | dict) -> None:
        ...
        if self.batched:
            ...
            if len(self.batch) >= self.batch_size:
                self.futures_executor.submit(with_context(self._safe_publish_event_payload_batch), self.batch)
                self._batch = []
        else:
            self.futures_executor.submit(with_context(self._safe_publish_event_payload), event_payload)
