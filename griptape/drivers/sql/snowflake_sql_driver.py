...
class SnowflakeSqlDriver(BaseSqlDriver):
    ...
    engine: Engine = field(
        default=Factory(
            ...
            lambda self: create_engine("snowflake://not@used/db", creator=self.connection_func),
            takes_self=True,
        ),
        kw_only=True,
    )
    ...
    def get_table_schema(self, table: str, schema: Optional[str] = None) -> Optional[str]:
        try:
            ...
            table = Table(table, metadata_obj, schema=schema, autoload=True, autoload_with=self.engine)
