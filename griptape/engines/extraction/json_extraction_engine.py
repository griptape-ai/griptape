...
class JsonExtractionEngine(BaseExtractionEngine):
    ...
    def extract(self, text: str | ListArtifact, template_schema: dict, **kwargs) -> ListArtifact | ErrorArtifact:
        try:
            ...
            return ListArtifact(
                self._extract_rec(
                    text.value if isinstance(text, ListArtifact) else [TextArtifact(text)], json_schema, []
                ),
                item_separator="\n",
            )
    ...
    def _extract_rec(
        self, artifacts: list[TextArtifact], json_template_schema: str, extractions: list[TextArtifact]
    ) -> list[TextArtifact]:
        artifacts_text = self.chunk_joiner.join([a.value for a in artifacts])
        full_text = self.template_generator.render(json_template_schema=json_template_schema, text=artifacts_text)
        ...
        if self.prompt_driver.tokenizer.count_tokens_left(full_text) >= self.min_response_tokens:
            ...
            partial_text = self.template_generator.render(template_schema=json_template_schema, text=chunks[0].value)
            ...
            return self._extract_rec(chunks[1:], json_template_schema, extractions)
