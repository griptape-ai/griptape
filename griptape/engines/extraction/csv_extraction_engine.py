...
class CsvExtractionEngine(BaseExtractionEngine):
    ...
    def extract(self, text: str | ListArtifact, column_names: list[str], **kwargs) -> ListArtifact | ErrorArtifact:
        try:
            return ListArtifact(
                self._extract_rec(
                    text.value if isinstance(text, ListArtifact) else [TextArtifact(text)], column_names, []
                ),
                item_separator="\n",
            )
    ...
    def _extract_rec(
        self, artifacts: list[TextArtifact], column_names: list[str], rows: list[CsvRowArtifact]
    ) -> list[CsvRowArtifact]:
        artifacts_text = self.chunk_joiner.join([a.value for a in artifacts])
        full_text = self.template_generator.render(column_names=column_names, text=artifacts_text)
        ...
        if self.prompt_driver.tokenizer.count_tokens_left(full_text) >= self.min_response_tokens:
            ...
            partial_text = self.template_generator.render(column_names=column_names, text=chunks[0].value)
