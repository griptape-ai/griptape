...
def remove_field_unless_directly_inside(d: dict, field_name: str, exception_container_key: str) -> dict:
    """Recursively remove a field_name from schema dicts unless it appears as a
    property name inside the exception_container_key (e.g., 'properties').

    Useful for stripping metadata keys like 'title' generated by Pydantic while
    preserving user-defined properties literally named 'title'.
    """
    def _filter_recursively(obj, inside_exception_container: bool = False):
        if not isinstance(obj, dict):
            return obj

        filtered_dict = {}
        for k, v in obj.items():
            if inside_exception_container:
                # iterating inside exception_container_key; keep keys untouched and recurse
                filtered_dict[k] = _filter_recursively(v, inside_exception_container=False)
            else:
                if k == exception_container_key and isinstance(v, dict):
                    filtered_dict[k] = _filter_recursively(v, inside_exception_container=True)  # recurse into the container
                elif k == field_name:
                    continue  # drop the key at this level
                else:
                    filtered_dict[k] = _filter_recursively(v, inside_exception_container=False)
        return filtered_dict

    return _filter_recursively(d, inside_exception_container=False)
